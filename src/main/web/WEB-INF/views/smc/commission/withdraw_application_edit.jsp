  ...
  <div class="body">
    <div class="page-body">
      <div class="page-title">
        编辑佣金提取申请
        <a href="javascript:showDialog()" seed=122225011 class="btn btn-creat">选择佣金结算申请</a>
      </div>
      <div class="modal-body">
        <h2 class="text-center">佣金提取申请单</h2>
        <form id="newWithDrawForm" onsubmit="formchongfu()" action="${contextPath}/commission-withdraw-applications?_method=put" method="post">
          <input type="hidden" id="status" name="status" />
          <input type="hidden" name="workFlowId" value="${withdraw.workFlowId}" />
          <input type="hidden" id="totalAmount" name="totalAmount" value="${withdraw.totalAmount}">
          <input type="hidden" name="applicationId" value="${withdraw.applicationId}">
          <input type="hidden" name="applicationCode" value="${withdraw.applicationCode}">
          <input type="hidden" id="totalForeignAmount" name="totalForeignAmount" value="${withdraw.totalForeignAmount}">
          <div class="row">
            <div class="creatTime">
              <label>申请日期:</label>
              <div class="input-group">
                <input seed=122225001 readonly="readonly" class="active" type="text" style="width: 100px;" id="applyDate" name="applicationDate" data-required="" />
              </div>
            </div>
          </div>
                ...
                <td style="text-align: left">
                  <c:choose>
                    <c:when test="${withdraw.businessMainBody == 1}">
                      <span id="ywzt">江苏跨境电子商务服务有限公司</span>
                      <input type="hidden" id="selectType" name="businessMainBody" value="1">
                      </td>
                    </c:when>
                    <c:when test="${withdraw.businessMainBody == 2}">
                      <span id="ywzt">南京贸互通电子商务服务有限公司</span>
                      <input type="hidden" id="selectType" name="businessMainBody" value="2">
                      </td>
                    </c:when>
                    <c:otherwise>
                      <span id="ywzt">南京贸互达科技有限公司</span>
                      <input type="hidden" id="selectType" name="businessMainBody" value="3">
                      </td>
                    </c:otherwise>
                </c:choose>
                <th>渠道商名称</th>
                <td colspan="3" style="text-align: left">
                  <span id="qds">${withdraw.channelName}</span>
                  <input type="hidden" id="channelName" name="channelName" value="${withdraw.channelName}">
                  <input type="hidden" id="channelId" name="channelId" value="${withdraw.channelId}">
                </td>
              </tr>
              <tr>
                <th style="line-height: 30px">收款人*</th>
                <td style="text-align: left; position:relative;">
                  <input seed=122225002 type="text" autocomplete="off" style="width: 245px" id="beneficiaryName" name="beneficiaryName" value="${withdraw.beneficiaryName}" data-required="">
                  <ul id='beneficiaryId'></ul>                
                </td>
                <th style="line-height: 30px">收款银行*</th>
                <td style="text-align: left">
                  <input seed=122225003 type="text" class="form-control" style="width: 245px" autocomplete="off" id="beneficiaryBank" name="beneficiaryBank" value="${withdraw.beneficiaryBank}" data-required="">
                </td>
              </tr>
              <tr>
                <th style="line-height: 30px">银行账号*</th>
                <td style="text-align: left">
                  <input seed=122225004 type="text" class="form-control" style="width: 245px" autocomplete="off" id="beneficiaryAccountNo" name="beneficiaryAccountNo" value="${withdraw.beneficiaryAccountNo}" data-required="" data-number0="">
                </td>
                <th style="line-height: 30px">是否提取为美元</th>
                <td style="text-align: left">
                  <c:choose>
                    <c:when test="${withdraw.currencyCode=='USD'}">
                      <input seed=122225005 type="checkbox" class="checkRate" checked="checked" onclick="getSellRate(this)">
                      <input type="hidden" class="currencyCode" name="currencyCode" value="USD">
                      <input type="hidden" id="currencyId" name="currencyId" value="318">是
                </td>
                </c:when>
                <c:otherwise>
                  <input seed=122225005 type="checkbox" class="checkRate" onclick="getSellRate(this)">
                  <input type="hidden" class="currencyCode" name="currencyCode" value="CNY">
                  <input type="hidden" id="currencyId" name="currencyId" value="151">是</td>
                </c:otherwise>
                </c:choose>
              </tr>
              <tr>
                ...
                <c:choose>
                  <c:when test="${withdraw.currencyCode=='USD'}">
                    <td style="text-align: left">
                      <input type="text" id="sellingRate" class="form-control" style="width: 245px" name="sellingRate" value="${withdraw.sellingRate}" readonly="readonly">
                    </td>
                  </c:when>
                  <c:otherwise>
                    <td style="text-align: left">
                      <input type="text" id="sellingRate" class="form-control" style="width: 245px" name="sellingRate" value="1" readonly="readonly">
                    </td>
                  </c:otherwise>
                </c:choose>
                <th style="line-height: 30px">备注</th>
                <td style="text-align: left">
                  <input seed=122225006 type="text" class="form-control" style="width: 245px" name="remark" value="${withdraw.remark}">
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <div class="btn-submit">
            <c:if test="${withdraw.status == 1}">
              <button type="button" seed=122225009 class="btn btn-default btn-font4" id="saveDraft">保存草稿</button>
            </c:if>
            <button type="submit" seed=122225010 class="btn btn-primary btn-font4" id="submitApply">提交申请</button>
          </div>
        </form>
      </div>
        ...
        <div class="modal-dialog" style="width: 1000px;">
          <div class="modal-content">
                ...
                <form id="settlementForm">
                  <div class="search-area">
                    <div class="search-group">
                      <label>结算申请单号:</label>
                      <input class="form-control" name="applicationCode" type="text" maxlength="30">
                    </div>
                    <div class="search-group">
                      <label>渠道商:</label>
                      <input class="form-control" id="searchChannelName" name="channelName" type="text" maxlength="30" readonly="readonly">
                    </div>
                    <div class="search-group">
                      <label>订单号:</label>
                      <input class="form-control" id="orderCodeW" name="orderCode" type="text" maxlength="30">
                    </div>
                    <div class="search-group">
                      <label>业务主体:</label>
                      <select name="businessMainBody" id="businessMainBody" disabled="disabled" class="form-control">
            ...
            <div class="modal-footer mhd-layer-footer">
              <button class="btn btn-primary btn-font4" type="button" onclick="saveSettlement()">确认选择</button>
            </div>
          </div>
        </div>
      </div>
...
<script type="text/javascript">
	...
	var withdrawDetail = '${withdrawDetail}';
	$(function() {
		newArr = withdrawDetail.split(",");
		loadData();
	});
	...
	function selectSingle(obj, type) {
		if ($(obj).is(":checked")) {
			...
			newArr.splice($.inArray($(obj).attr("settlement"), arr), 1);
			hits -= 1;
			if (hits == 0) {
				settlementId = "";
	...
	var businessMainBody = $('#businessMainBody').val();
	...
	function loadData() {
		...
		for (var i = arr.length - 1; i >= 0; i--) {
			...
			html += "<tr><td>"
					...
					+ "</td><td>"
					+ a[3]
					+ "<input type='hidden' name='withDrawdetails["+i+"].withdrawnAmount' value='"+a[3]+"'/></td>";
			if (a.length > 4) {
				html += "<td><input type='text' seed=122225007 class='form-control' amount='amount' name='withDrawdetails["+i+"].amount' value='"+a[4]+"' data-required data-maxvalidatenum='1' onkeyup='checkValue(this)'></td>";
			} else {
				html += "</td><td><input type='text' seed=122225007 class='form-control' amount='amount' name='withDrawdetails["+i+"].amount' data-required data-maxvalidatenum='1' onkeyup='checkValue(this)'></td>";
			}
			if (a.length > 4) {
				html += "<td><input type='text' class='form-control' readonly='readonly' foreignAmount='foreignAmount' name='withDrawdetails["+i+"].foreignAmount' value='"+a[5]+"'></td>";
			} else {
				html += "<td><input type='text' class='form-control' readonly='readonly' foreignAmount='foreignAmount' name='withDrawdetails["+i+"].foreignAmount' value='0.00'></td>";
			}
			html += "<td><a seed=122225008 onclick=\"new function(){newArr.splice($.inArray('"
					+ arr[i] + "',newArr),1);loadData();}\">移除</a></td>"
					+ "</tr>";
		}
		html += "<tr><th style='text-align: center;'>合计</th>"
				+ "<th colspan='3' style='text-align: right;'><span id='totalAmountTxt' style='font: 18px bolder'>"
				+ ${withdraw.totalAmount}+" CNY</span></th>"
				+ "<th style='text-align: right;'><span id='totalForeignAmountTxt' style='font: 18px bolder'>"
				+ ${withdraw.totalForeignAmount}+" USD</span></th>" + "<th></th></tr>";
	...
	function setForeignAmount(obj, amount) {
		...
		$(obj).parents("tr").find("td:eq(4) input:eq(0)").val(
				rate == 1 ? 0.00 : MFormat(parseFloat(amount) / rate, 2));
